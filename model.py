...
from utils.utils import bio_to_json
...
class Model(object):
    def __init__(self, config):
        ...
        # bert模型参数初始化的地方
        init_checkpoint = os.path.join(self.init_checkpoint, "bert_model.ckpt")
        # 获取模型中所有的训练参数。
    ...
    def bert_embedding(self):
        # load bert embedding
        bert_config = modeling.BertConfig.from_json_file(os.path.join(self.init_checkpoint, "bert_config.json"))
    ...
    def evaluate_line(self, sess, inputs, id_to_tag, export=True):
        trans = self.trans.eval(sess)  # 获取标签转移矩阵

        if export:
            tf.saved_model.simple_save(sess,
                                       "client/saved_model/000000/",  # 保存路径
                                       inputs={"input_ids": self.input_ids,
                                               "input_mask": self.input_mask,
                                               "segment_ids": self.segment_ids,
                                               "dropout": self.dropout},
                                       outputs={"output": self.logits})  # 输出接口
            np.save("client/vocab/trans.npy", trans)
            print("save success ...")
